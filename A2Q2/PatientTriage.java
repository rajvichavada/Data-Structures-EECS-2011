...
public class PatientTriage {
  ...
  /**
    ...
    public Patient remove(Time currentTime) throws NullPointerException, EmptyQueueException, BoundaryViolationException {
    	...
    	if (currentTime.getMinute() > 59 || currentTime.getMinute() < 0 || currentTime.getHour() > 23 || currentTime.getHour() < 0) 
    	{
    		throw new BoundaryViolationException();
    	}
    	//initialize a comparator 
    	Comparator<Time> ct = new TimeComparator();
    	//get elapsed time from current time
    	if (ct.compare(timeHeap.peek().getArrivalTime().elapsed(currentTime), this.maxWait) >= 0)	//If patient has waited for more than max time 
    	{
    		p = timeHeap.poll();	
    	
    		//priorityHeap.remove(p.getPriorityPos());	
    		
    		return p;
    	}
    	else 
    		 p = priorityHeap.poll();
			//timeHeap.remove(p.getPriorityPos());
			return p;
